...
from planners.planners import ProbabilisticRoadmap, Color, RRT, CircularObstacle
...
class App:
    def __init__(self):
        ...
        self.planners = ['Probabilistic Roadmap', "RRT"]
        self.default_planner = 'RRT'
        self.state = State.RRT

        self.searches = ['Dijkstra']
        self.default_search = 'Dijkstra'
        self.search = None
    ...
    def init_state(self):
        ...
        if self.state == State.PRM:
            ...
            self.t = threading.Thread(target=self.planner.set_sample_size, args=(self.prm_options['sample_size'],))
            self.t.start()
    ...
    def simulateState(self):
        if self.state == State.PRM:
            self.planner.set_search_algo(self.search)
            self.planner.prepare_PRM(self.map, self.node_radius, self.prm_options['neighbours'])
        if self.state == State.RRT:
